...
// The real Boggle dice are heavily vowel-loaded (one 5×5 die is AAEEEE — six
// vowel faces), which piles up A's and E's and makes boards feel unrandom.
// Target vowel counts a notch below the dice's natural average, and how hard to
// hunt for a good one. Bands are per grid; roughly 27–34% of tiles.
const VOWEL_BAND = {4: [4, 6], 5: [7, 9], 6: [10, 12]};
const GEN_TRIES  = {4: 16, 5: 24, 6: 40};
/* Score a board: 0 is ideal, higher is worse. Punishes vowel counts outside the
   band, three-plus of any one vowel (the A/E pileup), four-plus of a consonant,
   and same-letter clumps. */
function boardPenalty(board, adj, band){
  const counts = {};
  let vowels = 0;
  for (const c of board){ counts[c] = (counts[c]||0) + 1; if (VOWELS.has(c[0])) vowels++; }
  let p = 0;
  if (vowels < band[0]) p += (band[0] - vowels) * 3;
  if (vowels > band[1]) p += (vowels - band[1]) * 3;
  for (const k in counts){
    if (VOWELS.has(k[0])){ if (counts[k] > 2) p += (counts[k] - 2) * 5; }
    else if (counts[k] > 3) p += (counts[k] - 3) * 2;
  }
  p += Math.max(0, maxAdjacentCluster(board, adj) - 2) * 2;
  return p;
}
/* Roll several boards and keep the best-scoring one. This beats a pass/fail
   re-roll because the dice can't always satisfy every rule — best-of-N always
   lands as close as possible instead of falling through to a bad board. Fully
   deterministic in the seed (same rolls, same winner), so every phone in a
   party still shows the identical grid. */
function genBoard(seed, n){
  const band = VOWEL_BAND[n] || [Math.round(n*n*0.27), Math.round(n*n*0.34)];
  const tries = GEN_TRIES[n] || 24;
  const adj = adjacency(n);
  let best = null, bestP = Infinity;
  for (let a = 0; a < tries; a++){
    const board = rollBoard(a ? seed + '~' + a : seed, n);
    const p = boardPenalty(board, adj, band);
    if (p < bestP){ bestP = p; best = board; if (p === 0) break; }
  }
  return best;
}
...
/* Autoplay policy needs a real gesture — the first tap anywhere starts it. Uses
   click, not pointerdown: pointerdown fires and bubbles to this document-level
   listener BEFORE the SOUND button's own click handler has flipped P.sound, so
   tapping "off" as your very first-ever tap raced Music.start() ahead of the
   toggle and left a few notes already scheduled to ring for up to ~2s. click
   fires after the target's own handler, so by the time this listener sees it,
   P.sound already reflects what was actually tapped. */
